...
@section('content')
    <section class="p-4 bg-white rounded-xl lg:p-5 mb-5 shadow-lg">
        <div class="mb-5 flex justify-between items-center">
            ...
            <a href="{{ route('pegawai.tambah-pegawai') }}"
                        ...
                        <tr>
                            ...
                            <td class="lg:grid lg:grid-cols-2 lg:gap-5">
                                @if (strtolower($user->role) != 'pegawai')
                                    <form action="{{ route('pegawai.change-pegawai', ['id' => $user->id]) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit"
                                            class="mb-2 lg:mb-0 block p-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 duration-300 ease-in-out text-center">pegawai</button>
                                    </form>
                                @endif
                                <form id="delete-form-{{ $user->id }}"
                                    action="{{ route('pegawai.delete-pegawai', $user->id) }}" method="POST"
