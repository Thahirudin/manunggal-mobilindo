...
@section('title', 'Tambah Data Banner | ' . config('app.name'))
...
@section('content')
    <section class="p-4 bg-white rounded-xl lg:p-5 mb-5 shadow-lg">
        <div class="mb-5">
            <p class="text-xl font-bold">Tambah Data Banner</p>
        </div>
        <form action="{{ route('pegawai.store-banner') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-5">
                ...
                <input type="text" id="nama" name="nama"
                    class=" @error('nama') {{ 'border-red-500 ' }}@else {{ 'border-gray-300' }} @enderror bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Promosi Ramadhan" required />
                    @error('nama')
            ...
            <div class="mb-5">
                ...
                <input
                    ...
                    id="gambar" name="gambar" type="file" required accept="image/*">
                    @error('gambar')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
